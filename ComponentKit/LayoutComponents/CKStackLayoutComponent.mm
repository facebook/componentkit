...
+ (instancetype)newWithView:(const CKComponentViewConfiguration &)view
  ...
  {
    CKStackLayoutComponent * const component = [super newWithView:view size:size];
    if (component) {
  ...
  static YGFlexDirection cssDirectionFromStackStyle(const CKStackLayoutComponentStyle &style)
  {
  ...
  static YGJustify cssJustifyFromStackStyle(const CKStackLayoutComponentStyle &style)
  {
  ...
  static YGAlign cssAlignFromStackStyle(const CKStackLayoutComponentStyle &style)
  {
  ...
  static YGAlign cssAlignFromChild(const CKStackLayoutComponentChild &child)
  {
...
- (YGNodeRef)cssStackLayoutNode:(CKSizeRange)constrainedSize cache:(NSArray<CKFlexboxChildCachedLayout *> **)layoutCache
  {
    const YGNodeRef stackNode = YGNodeNew();
    ...
    for (auto iterator = children.begin(); iterator != children.end(); iterator++) {
      ...
      const YGNodeRef childNode = YGNodeNew();
      ...
      YGNodeStyleSetAlignSelf(childNode, cssAlignFromChild(child));
    ...
    YGNodeStyleSetFlexDirection(stackNode, cssDirectionFromStackStyle(_style));
    YGNodeStyleSetJustifyContent(stackNode, cssJustifyFromStackStyle(_style));
    YGNodeStyleSetAlignItems(stackNode, cssAlignFromStackStyle(_style));
    return stackNode;
  }
...
- (CKComponentLayout)computeLayoutThatFits:(CKSizeRange)constrainedSize
  {
    ...
    for (NSUInteger i = 0; i < childCount; i++) {
      sortedChildNodes[i] = YGNodeGetChild(layoutNode, (uint32_t)i);
    }
    ...
    return {self, {width, height}, childrenLayout};
  }
